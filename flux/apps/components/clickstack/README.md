...
| FerretDB → Postgres | `ferretdb-rw.clickstack.svc:5432/ferretdb` (user `ferretdb`, password from `ferretdb-app-secret`) |
| HyperDX → ClickHouse | `DEFAULT_CONNECTIONS` (`connections.json`): `http://clickstack-clickstack.clickstack.svc:8123`, user `default` |
| Collector → ClickHouse | `CLICKHOUSE_ENDPOINT=tcp://clickstack-clickstack.clickstack.svc:9000?dial_timeout=10s`, user `default` |
...
(32 random bytes) from Proton Pass. The `clickstack` Zitadel client is owned by this app's `clickstack-sso` Terraform CR (no `org_id` literal in git).
...
- Updates via `flux/apps/update-policies/clickstack.yaml` (ImageRepository + ImagePolicy + `$imagepolicy` markers; proxy markers shared with hubble-ui + flux-operator-ui — bump together). Version sources: `base/clickstack.yaml` (HyperDX app + collector), `base/ferretdb.yaml` (FerretDB), `base/oauth2-proxy.yaml` (chart + app). Snapshot the `ferretdb` CNPG cluster + confirm a ClickHouse `BACKUP ALL` before major bumps.
- Changelogs: HyperDX https://github.com/hyperdxio/hyperdx/releases · FerretDB https://github.com/FerretDB/FerretDB/releases · proxy chart https://github.com/oauth2-proxy/manifests/releases · proxy image https://github.com/oauth2-proxy/oauth2-proxy/releases.
