...
- **Upgrade lifecycle (check versions -> check changelog -> bump -> migrate/verify).** Every bump follows the same manual loop: check the available version (policy `range:` floor plus upstream tags), read the changelog first (canonical pages under Reference), bump the pin, then migrate values/CRDs and verify with the gates. Dev soaks first; promote dev->stable/prd only after dev is green. Automation proposes (ImageUpdateAutomation PR), human merges after review — never auto-merge. Per-type mechanics: OCI chart/image — let the `$imagepolicy` marker move via the automation PR, then hand-verify; classic `HelmRepository` (vpa, seaweedfs-operator, seaweedfs-csi-driver) — hand-bump `version:`/`tag:` plus the policy floor, never add a new classic source; vendored bundles (KubeVirt operator+CR, Multus) — re-download at the new tag, diff, then bump the marker; provider/ISO/schematic (Netbird `versions.tf` `~>`, Talos `talos_version`, `tofu-controller` digest) — bump version and digest together. Chart<->app divergence checklist: coder chart vs app image, zitadel chart vs app, coredns chart vs app, clickhouse-operator vs server/keeper, rclone-sync chart vs appVersion (all six wrappers atomically), `tofu-controller` chart vs `tofu-runner`, oauth2-proxy chart 10.7.0 vs app image v7.15.4 (move chart + image markers together). Coupled moves: Cilium<->Gateway API, Hubble UI<->Cilium, flux-operator-ui chart tag <-> `flux/fleet/terraform/versions.yaml` `operator_chart_version`, clickhouse-server<->keeper, oauth2-proxy image (`apps:oauth2-proxy:tag`) + chart (`apps:oauth2-proxy-chart:tag`) shared across clickstack/hubble-ui/flux-operator-ui with seaweedfs ui-auth on its own `infra:seaweedfs-ui-auth(-chart):tag` pair.
- **Helm OCI-first; official preferred, mirror-only for 3rd-party.** New charts use `OCIRepository` + `chartRef` (interval 1h) — OCI always beats classic `HelmRepository`. Prefer the upstream official OCI chart; 3rd-party/community charts are allowed only via the home-operations OCI mirror `oci://ghcr.io/home-operations/charts-mirror/*` (repo https://github.com/home-operations/charts-mirror, local docs `/tmp/home-ops-docs/home-operations-oci-helm-chart-mirror-docs/README.md`: stop-gap, cosign-signed, pruned 6 months after upstream goes OCI — switch back to official then). Examples now on OCI: coder (official `ghcr.io/coder/chart/coder`), coredns (official `ghcr.io/coredns/charts/coredns`), headlamp (mirror `ghcr.io/home-operations/charts-mirror/headlamp`). Only grandfathered classic exceptions: vpa `vertical-pod-autoscaler` (https://kubernetes.github.io/autoscaler), seaweedfs-operator, seaweedfs-csi-driver — each keeps a header comment stating why it is still classic.
