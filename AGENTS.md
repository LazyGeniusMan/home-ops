...
- **Observability (ClickHouse-native).** ClickStack/HyperDX + ClickHouse is the only store; HyperDX is the only UI — no Prometheus/Grafana. `ServiceMonitor`/`PodMonitor` are target discovery for the OTel pipeline only: keep off by default, the only sanctioned on-state is a chart-native guard (`renderMode: skipIfMissing`, e.g. `infra/components/external-secrets/controllers/base/externalsecrets.yaml` `serviceMonitor` block), every flip carries a why-comment. Go standard: JSON `slog` + OTel trace SDK v1.46.0 (`internal/tracing`, OTLP/HTTP `:4318`, `trace_id` in logs, `/metrics` kept, `OTEL_SDK_DISABLED` bypass). Probes: `/healthz` + `/readyz` on every Deployment, values tuned per component, `startupProbe` for slow starts (Zitadel 60s/30s). VPA `Off` with HPA, `Initial` for singletons (external-dns, tofu-controller). Alerts: HyperDX webhook → `apprise-go-api` → Matrix. Upgrades follow changelog-first + pin parity (ClickHouse 26.8 LTS lockstep, OTel SDK across all Go services, oauth2-proxy chart/app together). Full per-signal guide: `flux/OBSERVABILITY.md`.
