...
- **Observability (ClickHouse-native).** ClickHouse is the only store, HyperDX the only UI — no Prometheus/Grafana. `ServiceMonitor`/`PodMonitor` stay off by default; every flip carries a why-comment. Go services log JSON `slog` with OTel traces; every Deployment probes `/healthz` + `/readyz`. VPA `Off` alongside HPA, `Initial` for singletons. Alerts flow HyperDX webhook → `apprise-go-api` → Matrix. Per-signal values, versions, and upgrade locksteps: `flux/OBSERVABILITY.md`.
