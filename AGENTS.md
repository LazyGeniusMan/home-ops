...
- Read the docs that describe current state before acting, in this order: the relevant `README.md` / `RUNBOOK.md` next to the code, then reference docs under `/tmp/home-ops-docs`, then the web. Refresh references by hand (`scripts/fetch-references.sh -m zip`; see script help for modes).
...
- Toolchain source of truth is `.flox/env/manifest.toml`; `talosctl` is curl-fetched by the `on-activate` hook into `.flox/cache/bin`, not from the catalog. On any tool upgrade, migrate every consumer together so pins keep parity across the Flox manifest, `talos/ansible/group_vars/all.yml`, Terraform/Ansible version constraints, GitHub workflows, Dockerfiles, and Flux manifests.
...
- **Upgrade lifecycle (check versions -> check changelog -> bump -> migrate/verify).** Check the available version (policy `range:` floor plus upstream tags), read the changelog first (see Changelog-first under Reference), bump the pin, then migrate values/CRDs and verify with the gates. Dev soaks first; promote dev->stable/prd only after dev is green. Per-type mechanics: OCI chart/image and chartproxy-proxied classic-only charts (headlamp, vpa, seaweedfs-operator, seaweedfs-csi-driver) — let the `$imagepolicy` marker move via the automation PR, then hand-verify; vendored bundles (KubeVirt operator+CR, Multus) — re-download at the new tag, diff, then bump the marker; provider/ISO/schematic (Netbird `versions.tf` `~>`, Talos `talos_version`, `tofu-controller` digest) — bump version and digest together. Chart<->app pairs move together (coder chart vs app image, zitadel chart vs app, coredns chart vs app, clickhouse-operator vs server/keeper, rclone-sync chart vs appVersion across all six wrappers atomically, `tofu-controller` chart vs `tofu-runner`, oauth2-proxy chart markers + image markers together). Coupled moves: Cilium<->Gateway API, Hubble UI<->Cilium, flux-operator-ui chart tag <-> `flux/fleet/terraform/versions.yaml` `operator_chart_version`, clickhouse-server<->keeper, oauth2-proxy image (`apps:oauth2-proxy:tag`) + chart (`apps:oauth2-proxy-chart:tag`) shared across consumers with seaweedfs ui-auth on its own `infra:seaweedfs-ui-auth(-chart):tag` pair.
- **Helm OCI-first; official preferred, chartproxy for classic-only.** New charts use `OCIRepository` + `chartRef` (interval 1h); no classic `HelmRepository` is allowed. Prefer the upstream official OCI chart; if official publishes classic-only, use the chartproxy.container-registry.com proxy (`oci://chartproxy.container-registry.com/<host>[/subpath]/<chart>`, local docs `/tmp/home-ops-docs/helm-charts-oci-proxy`); no third-party/community mirrors. Chartproxy mappings carry a header comment stating the upstream classic source they proxy.
...
- **Workload standards (all values tuned per component docs):**
  ...
  - Resources (requests/limits) on every Deployment.
...
- **Observability (ClickHouse-native).** ClickStack/HyperDX + ClickHouse is the only store; HyperDX is the only UI — no Prometheus/Grafana. `ServiceMonitor`/`PodMonitor` are target discovery for the OTel pipeline only: keep off by default until flipped; the sanctioned on-state is `enabled: true` + `otel-scrape: "true"` label via the gateway TA into ClickHouse (namespace label from the fleet tenants template, monitor label via chart label knobs / overlay patches; ESO `renderMode: skipIfMissing` stays the guarded-chart template), every flip carries a why-comment. Go standard: JSON `slog` + OTel trace SDK v1.46.0 (`internal/tracing`, OTLP/HTTP `:4318`, `trace_id` in logs, `/metrics` kept, `OTEL_SDK_DISABLED` bypass). Probes: `/healthz` + `/readyz` on every Deployment, values tuned per component, `startupProbe` for slow starts (Zitadel 60s/30s). VPA `Off` with HPA, `Initial` for singletons (external-dns, tofu-controller). Alerts: HyperDX webhook → `apprise-go-api` → Matrix. Upgrades follow changelog-first + pin parity (ClickHouse 26.8 LTS lockstep, OTel SDK across all Go services, oauth2-proxy chart/app together). Full per-signal guide: `flux/OBSERVABILITY.md`.
...
- Do NOT manage Talos with anything but `talosctl` v1.15 semantics. No third-party provisioners, no hand-built machine configs.
